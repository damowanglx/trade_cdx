...
from data.fetcher.tushare_api import DataFetcher
...
def main():
    ...
    if daily_data.empty:
        print("获取数据失败，请检查Tushare Token配置")
        print("请在 config/settings.py 中设置你的Tushare Pro Token")
        return
    ...
    print("\n前5条数据:")
    print(daily_data[['open', 'high', 'low', 'close', 'vol']].head())
    ...
    # 6. 保存数据
    print("\n[步骤6] 保存数据...")
    fetcher.save_to_csv(daily_data, f'{ts_code}_with_indicators.csv')
    ...
    print("="*60)
    print("\n生成的文件:")
    print("  - data/cache/000001.SZ_daily_with_indicators.csv (数据文件)")
