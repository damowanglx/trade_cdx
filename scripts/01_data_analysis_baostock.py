...
def main():
    ...
    print("="*60)
    print("量化交易入门 - 数据获取与可视化（使用Baostock）")
    print("="*60)
    print("\n[OK] 使用Baostock：完全免费，无需注册，无需Token！")
    ...
    # 2. 获取股票数据
    print("\n[步骤2] 获取平安银行(sz.000001)最近1年数据...")
    symbol = 'sz.000001'  # 平安银行（深圳股票用sz前缀）
    ...
    # 6. 保存数据
    print("\n[步骤6] 保存数据...")
    fetcher.save_to_csv(daily_data, '000001_daily_with_indicators.csv')
