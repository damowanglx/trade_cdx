"""
全量股票回测 - 三策略比较
测试双均线、RSI、MACD三个策略在所有股票上的表现

使用方法：
    python scripts/run_batch_backtest_all.py
"""
...
def main():
    ...
    print("三策略比较结果")
    print("="*60)
    print(f"\n{'策略':<10} {'平均收益率':<12} {'中位数收益率':<14} {'盈利比例':<10} {'夏普比率':<10}")
    print("-"*60)
    ...
    for strategy_name, df in all_results.items():
        ...
        print(f"{strategy_name:<10} {avg_return:<12.2f} {median_return:<14.2f} {win_rate:<10.1f} {avg_sharpe:<10.2f}")
    
    # 找出最佳策略
    best_strategy = max(all_results.items(), key=lambda x: x[1]['total_return'].mean())
    print(f"\n最佳策略: {best_strategy[0]} (平均收益率: {best_strategy[1]['total_return'].mean()*100:.2f}%)")
    
    print("\n" + "="*60)
    print("全量回测完成！")
    print("="*60)
    print(f"\n结果文件:")
    print(f"  - reports/batch_双均线.csv")
    print(f"  - reports/batch_RSI.csv")
    print(f"  - reports/batch_MACD.csv")
