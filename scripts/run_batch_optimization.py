...
def optimize_strategy(strategy_class, param_grid, strategy_name, sample_size=200):
    """优化策略参数"""
    print(f"\n{'='*60}")
    ...
    print(f"预计时间: {len(param_combinations) * len(sample_files) * 0.05 / 60:.0f} 分钟")
    ...
    for i, values in enumerate(param_combinations):
        ...
        if i % 10 == 0:
            print(f"\n进度: {i}/{len(param_combinations)} ({i/len(param_combinations)*100:.1f}%)")
            print(f"当前参数: {params}")
    ...
    # 打印结果
    print(f"\n{'='*60}")
    print(f"{strategy_name}策略优化结果（按平均收益率排序）")
    print(f"{'='*60}")
...
def main():
    ...
    # 优化RSI策略
    rsi_results = optimize_strategy(RSIStrategy, rsi_param_grid, 'RSI', sample_size=200)
    
    # 优化MACD策略
    macd_results = optimize_strategy(MACDStrategy, macd_param_grid, 'MACD', sample_size=200)
    ...
    rsi_df['params'] = rsi_df['params'].astype(str)
    rsi_df.to_csv('reports/rsi_optimization_full.csv', index=False, encoding='utf-8-sig')
    
    # 保存MACD优化结果
    macd_df = pd.DataFrame(macd_results)
    macd_df['params'] = macd_df['params'].astype(str)
    macd_df.to_csv('reports/macd_optimization_full.csv', index=False, encoding='utf-8-sig')
    
    print("\n" + "="*60)
    print("参数优化完成！")
    print("="*60)
    print(f"\n结果文件:")
    print(f"  - reports/rsi_optimization_full.csv")
    print(f"  - reports/macd_optimization_full.csv")
